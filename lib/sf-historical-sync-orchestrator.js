...
async function runHistoricalSync(supabase, args) {
  ...
  try {
    logger.log(`[sf-historical-sync] tenant=${tenantId} lb_user=${lbUserId} dry_run=true sync_scope=${syncScope} status=${status || '-'} fetched=${summary.fetched_from_lb} would_link=${summary.would_link} (lost_truth_override=${summary.lost_truth_override_candidates}) would_review=${summary.would_review} already_linked=${summary.already_linked} would_skip=${summary.would_skip} more_may_exist=${summary.more_may_exist}`);
  } catch (_) {}
...
/**
 * Translate one categorize() output into the LB /link-leads-bulk row
 * shape for a feedback post. Returns null when this category should not
 * be posted (would_link/already_linked/skipped reasons).
 *
 * The matcher's representative job (when present) is reported back to
 * LB as `sf_job_id` even for `needs_review` / `low_confidence` outcomes
 * — LB does not persist it on those rows but echoes it back in the
 * response `detail` so an operator can see SF's best guess when they
 * later resolve the review.
 *
 * @param {object} args
 * @param {object} args.lbCandidate   - LB camelCase candidate
 * @param {object} args.categorized   - return value from categorize()
 * @returns {object|null}
 */
...
async function runHistoricalFeedbackApply(supabase, args) {
  ...
  try {
    ...
    // 4. For each candidate, run matcher + categorize, then build the
    //    feedback row (if the category is in allowedClasses).
    ...
    for (const lbCandidate of candidates) {
      ...
      if (cat.bucket === 'would_link')          summary.categorized_would_link++;
      ...
      const catReason = cat.reason || (cat.bucket === 'would_skip' ? 'no_match' : null);
      if (!allowedClasses.has(catReason)) {
      ...
      if (row.confidence === LB_CONF.NONE) summary.would_no_match++;
    ...
    const lbResponse = await linkLeadsBulk({
    ...
    try {
      logger.log(`[sf-historical-feedback] tenant=${tenantId} lb_user=${lbUserId} APPLIED applied=${applied.length} rejected=${rejected.length} request_id=${lbResponse.request_id || '-'}`);
    } catch (_) {}
