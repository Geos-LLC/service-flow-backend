...
function categorize({ lbCandidate, matched }) {
  ...
  // ── sf_truth_overrides_lb_automation_lost (NEW) ──
  //
  // LB does NOT decide if a person is still a lead vs already a client —
  // SF decides based on customer/job truth. When LB marked a lead `lost`
  // via automation (typical case: `lostReason='hired_someone'` after a
  // follow-up timeout) but SF has a high-confidence customer with a
  // completed+paid job, SF truth wins.
  //
  // Strict gate (all must hold) — fail-closed if any field is missing:
  //   1. lbCandidate.status === 'lost'
  //   2. lbCandidate.statusSource === 'lb_automation'  (NOT human-set)
  //   3. exactly 1 matched SF customer
  //   4. confidence >= 'high'                          (phone OR email exact)
  //   5. no ambiguity_warnings
  //   6. sf_job.status === 'completed'
  //   7. sf_job.payment_status === 'paid'
  //   8. matched customer NOT already reconciled       (no remap)
  //   9. matched job has no DIFFERENT lb_lead_id pinned
  //
  // Position: AFTER lb_already_pinned_to_different_job (the LB-side veto
  // is stricter — never override a deliberate LB pin disagreement) and
  // BEFORE already_reconciled_customer (otherwise an automation-false-
  // lost row whose customer happens to have another linked SF job would
  // get suppressed; but the strict gate at #8 makes that path correct).
  //
  // Until LB exposes `statusSource` on the candidate contract this rule
  // never fires (statusSource undefined !== 'lb_automation'). That's the
  // intended fail-closed behavior — those rows remain in would_skip
  // until LB ships the field.
  if (single
      && lbCandidate
      && lbCandidate.status === 'lost'
      && lbCandidate.statusSource === 'lb_automation'
      && (CONF_RANK[single.confidence] || 0) >= CONF_RANK.high
      && (!Array.isArray(single.ambiguity_warnings) || single.ambiguity_warnings.length === 0)
      && single.sf_job
      && single.sf_job.status === 'completed'
      && single.sf_job.payment_status === 'paid'
    ...
    return { bucket: 'would_link', reason: 'sf_truth_overrides_lb_automation_lost', matched: matches };
  }
...
async function runHistoricalSync(supabase, args) {
  ...
  for (const lbCandidate of candidates) {
    ...
    if (cat.bucket === 'would_link') {
      ...
      summary.would_link++;
      if (cat.reason === 'sf_truth_overrides_lb_automation_lost') {
        summary.automation_false_lost_candidates++;
      }
  ...
  try {
    logger.log(`[sf-historical-sync] tenant=${tenantId} lb_user=${lbUserId} dry_run=true sync_scope=${syncScope} status=${status || '-'} fetched=${summary.fetched_from_lb} would_link=${summary.would_link} (auto_false_lost=${summary.automation_false_lost_candidates}) would_review=${summary.would_review} already_linked=${summary.already_linked} would_skip=${summary.would_skip} more_may_exist=${summary.more_may_exist}`);
  } catch (_) {}
...
function isApplicable({ lbCandidate, matched }) {
  ...
  // Defense-in-depth for the sf_truth_overrides_lb_automation_lost
  // categorize() rule. Preview surfaces lost rows as would_link only when
  // (a) LB's lost was set by automation, AND (b) the matched SF job is
  // completed+paid. isApplicable() re-checks both so a stale operator
  // approval (e.g. queued before LB started exposing statusSource) can't
  // slip a downgrade through.
  if (lbCandidate.status === 'lost') {
    if (lbCandidate.statusSource !== 'lb_automation') {
      return { ok: false, reason: 'lost_not_lb_automation_origin' };
    }
    if (!c.sf_job || c.sf_job.status !== 'completed' || c.sf_job.payment_status !== 'paid') {
      return { ok: false, reason: 'lost_requires_completed_paid_sf_job' };
    }
  }
