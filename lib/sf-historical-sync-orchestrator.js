...
function categorize({ lbCandidate, matched }) {
  ...
  // LB-already-pinned-to-different-job guard.
  //
  // LB's bulk-link safeguard refuses to overwrite an existing
  // `leads.sfJobId` with a different value (no-overwrite-different-sfJobId).
  // The conflict-rejection pattern we observed in Batches #1B/#4B/#6 was
  // 100% explained by this single LB-side invariant: when LB has the lead
  // pinned to a cancelled-originator job and SF's tiered matcher picks a
  // later completed+paid job for the same customer, LB returns
  // `result: conflict`.
  //
  // The deterministic predicate is the LB-side pin disagreement, NOT
  // `lbCandidate.status === 'cancelled'` (which is just a correlate). The
  // LB candidates endpoint already returns `sfJobId` per row, so SF can
  // observe the disagreement during planning and exclude these rows from
  // apply manifests.
  //
  // Position: BEFORE the already_reconciled_customer guard, because this
  // is a stricter observable veto (LB has made a state decision we can
  // see) — not an inference about SF-side reconciliation history.
  ...
  // Already-reconciled-customer guard.
  //
  ...
  //
  // Fires when either:
  //   - sf_customer.lb_lead_id is set (customer row carries a link), OR
  //   - sf_customer.any_job_linked is true (at least one of the
  //     customer's SF jobs has lb_lead_id set)
  //
  // Position: BEFORE the sf_job_linked_to_different_lb_lead conflict
  // check, because already-reconciled is a stronger signal — the
  // remediation path for these is "leave the existing link alone",
  // not "surface for operator review of a conflict".
