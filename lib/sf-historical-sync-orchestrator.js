...
async function runHistoricalSync(supabase, args) {
  ...
  try {
    logger.log(`[sf-historical-sync] tenant=${tenantId} lb_user=${lbUserId} dry_run=true fetched=${summary.fetched_from_lb} would_link=${summary.would_link} would_review=${summary.would_review} would_skip=${summary.would_skip} more_may_exist=${summary.more_may_exist}`);
  } catch (_) {}
