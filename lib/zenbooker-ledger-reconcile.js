 ...
 */
async function computeIntendedRows(supabase, jobId, userId) {
  ...
  if (!job) return { eligible: false, reason: 'job_not_found', intended: [], job: null };
  ...
  for (const member of teamMembers) {
    ...
    const eff = memberRates.length > 0
      ? getEffectivePayRate(memberRates, effectiveDate)
      : { hourlyRate: num(member.hourly_rate), commissionPercentage: num(member.commission_percentage) };
 ...
 */
async function safeReconcileJobLedger(supabase, { jobId, userId, dryRun = false }) {
  const computed = await computeIntendedRows(supabase, jobId, userId);
  const result = {
    job_id: jobId,
