...
// Outcome string used in the operational-outcome payload — minimal,
// post-2A-correction. LB receives outcome semantics, not SF-internal
// lifecycle states.
...
async function recordOrchestrationOutbound(supabase, args) {
  ...
  const outcome = OUTCOME_BY_EVENT_TYPE[args.eventType];

  // Operational-outcome payload only — no SF-internal lifecycle fields
  // (no jobs.status), no internal IDs that LB doesn't need. The minimal
  // contract is: which job, when did it happen, what's the outcome.
  const payload = {
    event_id: eventId,
    ...
    job: {
      ...
      scheduled_start: job.scheduled_date || null,
      outcome,
    },
