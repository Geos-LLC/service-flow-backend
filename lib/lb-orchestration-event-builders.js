...
//
// event_id format: evt_credential_rotated_<tenant>_<current_active_id>_refresh_<epoch_seconds>
//   The `_refresh_<ts>` suffix makes each mark cycle a unique event_id
//   on LB's receiver dedup. Same outbox row stores the same event_id on
//   retries (drainer doesn't regenerate per-attempt), so SF→LB retries
//   still hit LB's idempotent_replay path correctly. But two distinct
//   mark_for_rotation calls (e.g. operator marks, LB acknowledges,
//   operator clears + re-marks for a fresh test) produce distinct
//   event_ids → LB processes each one freshly.
// ─────────────────────────────────────────────────────────────────
function buildCredentialRotationRequiredEvent(args) {
  ...
  const tsSec = Number.isFinite(args.tsSeconds)
    ? args.tsSeconds
    : Math.floor(Date.now() / 1000);
  const eventId = `evt_credential_rotated_${args.tenantId}_${args.currentCredentialId}_refresh_${tsSec}`;
  return envelope(eventId, 'credential.rotated', args.tenantId, {
