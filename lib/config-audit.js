...
function inspectConfig(env = process.env) {
  ...
  // Sigcore is APP-LEVEL infrastructure, not tenant configuration. Without
  // these values the OpenPhone/Sigcore connect path (POST /api/communications/
  // connect-openphone) fails with HTTP 500 "Sigcore workspace key not
  // configured" for every tenant — feature outage with no surface signal
  // beyond a per-call 500. The boot loader (server.js, near SIGCORE_URL
  // declaration) ALSO accepts a DB row in communication_settings keyed by
  // sigcore_tenant_id='global_workspace', but the env vars remain the
  // preferred path because they survive DB-row corruption / unintended
  // upsert clobbers AND make the config visible in Railway's variables UI.
  if (!env.SIGCORE_URL) {
    findings.push({
      severity: 'HIGH',
      key: 'SIGCORE_URL',
      reason: 'SIGCORE_URL is unset — OpenPhone/Sigcore connect endpoint cannot resolve the Sigcore base URL via env fallback. If a global_workspace row exists in communication_settings, the boot loader will populate it; this warning persists because env fallback is the preferred persistent platform config.',
      fix: 'Set SIGCORE_URL=https://sigcore-production.up.railway.app/api on Railway. Optionally also write a communication_settings row with sigcore_tenant_id=global_workspace (loaded at boot).',
    });
  }

  if (!env.SIGCORE_WORKSPACE_KEY) {
    findings.push({
      severity: 'HIGH',
      key: 'SIGCORE_WORKSPACE_KEY',
      reason: 'SIGCORE_WORKSPACE_KEY is unset — POST /api/communications/connect-openphone returns 500 ("Sigcore workspace key not configured") for every tenant attempting to connect OpenPhone unless a communication_settings global_workspace row provides the key. Sigcore is app-level infrastructure; tenants should never need to know this value.',
      fix: 'Set SIGCORE_WORKSPACE_KEY to the platform Sigcore service key on Railway. Pull from AWS Secrets Manager sigcore-prod-secrets → SIGCORE_SERVICE_KEY. Optionally also save via Admin Global Settings (writes the global_workspace row).',
    });
  }
