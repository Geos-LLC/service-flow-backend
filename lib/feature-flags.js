...
const FLAGS = {
  ...
  //
  // Flag flip order: this flag must be ON for a tenant BEFORE
  // IDENTITY_RESOLVER_LEADBRIDGE is ON for the same tenant. Otherwise
  // the first repeat LB event enriches the canonical and acquisition
  // data is lost.
  LEAD_CARDINALITY_CHILD_LEADS: 'LEAD_CARDINALITY_CHILD_LEADS',
  // Identity reconciliation engine — Stage 1 (dark code) gates.
  // RECONCILIATION_ENGINE_AVAILABLE: foundation flag. The engine module
  //   loads regardless; this flag is reserved for future "engine ready
  //   to receive calls" signaling. Default OFF; no behavior change today.
  // RECONCILIATION_ENGINE_{LEADBRIDGE,ZENBOOKER,OPENPHONE,MANUAL_SF,SIGCORE}:
  //   per-source adapter flips. Stage 2/3/4/4.5/5 in
  //   docs/architecture/identity-graph-refactor-plan.md. Per-tenant via
  //   <FLAG>_TENANTS=2,7. ALL DEFAULT OFF and unused until adapters land.
...
// Per-tenant rollout helper. Reads `<FLAG>_TENANTS` env var as comma-separated
// integer user IDs; if userId is in the list the flag is ON for that tenant
// regardless of the global flag. The global flag is also respected (global ON
// applies to everyone). Used for staged identity-resolver rollout.
//
//   IDENTITY_RESOLVER_ZENBOOKER_TENANTS=2,7
//
// isEnabledForTenant(FLAGS.IDENTITY_RESOLVER_ZENBOOKER, 2) → true
// isEnabledForTenant(FLAGS.IDENTITY_RESOLVER_ZENBOOKER, 9) → false (unless global ON)
...
module.exports = { FLAGS, isEnabled, isEnabledForTenant, snapshot, envInt, getOpenPhoneLeadMaxAgeDays };
