...
const FLAGS = {
  ...
  // Returns sanitized completed-job pins (approximate lat/lng, city, service
  // type, completed month/year) read from the public_job_map_projection
  // table — never from the raw jobs table. Off by default. When OFF, the
  // endpoint returns { enabled: false, pins: [] } with HTTP 200 so embedders
  // render a benign empty map rather than an error. Additionally, each
  // tenant must have widget enabled in tenant_widget_settings; tenant-level
  // disable takes effect immediately regardless of the global flag.
