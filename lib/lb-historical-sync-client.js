...
async function linkLeadsBulk(args) {
  ...
  // LB's Prisma schema stores sf_job_id / sf_customer_id as String columns.
  // Passing them as JS numbers causes Prisma to drop the field from the
  // update payload (server-side detail: empty `data: {}` block →
  // result:'failed'). Stringify at the wire boundary so the orchestrator
  // can continue to carry numeric IDs internally.
