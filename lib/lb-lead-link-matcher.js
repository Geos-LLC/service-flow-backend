 ...
 *
 * Tiered selection (per post-Batch-5 spec):
 *   tier 1 — earliest `completed` + `payment_status=paid` job
 *   tier 2 — earliest `completed` job (any payment status)
 *   tier 3 — earliest `scheduled` / `booked` job
 *            (fires only when no completed job exists, so it
 ...
 * is "the lead converted into THIS particular completed service",
 * and the earliest completed/paid job is that signal.
 *
...
function pickHistoricalRepresentativeJob(jobs) {
  ...
  // tier 2: earliest completed (any payment)
