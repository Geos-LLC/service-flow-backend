 ...
 *   3. Else, if the source surfaced adjustment data (hasAdjustmentSignal) AND
 *      amount_paid exceeds (subtotal + taxes + adjustment_total), the overage
 *      is an implicit tip → use computed value.
 *      Without adjustment signal, we cannot distinguish "customer tipped" from
 *      "processing fee" — both produce identical overages — so we refuse to
 *      guess (omit tip_amount, preserving whatever SF has).
