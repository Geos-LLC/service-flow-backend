 ...
 *   3. Else, if the source surfaced adjustment data (hasAdjustmentSignal) →
 *      write tip = 0. (Previously this branch treated `amount_paid - subtotal
 *      - taxes - adjustment_total > 0` as an implicit tip. That assumption
 *      proved wrong for ZB invoices where the merchant's processing fee is
 *      collected from the customer but NOT recorded in `adjustment_total` —
 *      the overage was the fee, not a tip, but we credited cleaners with it.
 *      Incident: Tatiana Larionova Jun 2026, ~55 jobs. We now refuse to guess.)
 *   4. Else (no adjustment signal) → preserve SF (omit), can't tell anything.
 *
 * The function returns:
 *   - resolvedTip: number | null   (null = "do not write tip_amount")
 *   - tipSource:   one of 'explicit_zb' | 'preserve_sf' | 'no_adjustment_signal' | 'no_tip'
 */
function resolveTip({ explicitZbTip, existingSfTipAmount, amountPaid, subtotal, taxes, adjustmentTotal, hasAdjustmentSignal }) {
  ...
  if (!hasAdjustmentSignal) {
    return { resolvedTip: null, tipSource: 'no_adjustment_signal' };
  }
  // Previously: implicit overage → tip. Removed — see docstring.
