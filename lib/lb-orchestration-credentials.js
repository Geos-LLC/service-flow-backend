...
// ─────────────────────────────────────────────────────────────────
// Mark for rotation (R1B) — operator-set marker for pull-style rotation
// ─────────────────────────────────────────────────────────────────

/**
 * Mark the tenant's active credential as needing refresh. Sets
 * needs_refresh_at on the active row. Does NOT mint a new credential
 * or change status — LB triggers the actual rotation by calling
 * /credentials/refresh.
 *
 * Returns:
 *   { ok: true, credentialId, needsRefreshAt }    — marker set
 *   { ok: false, reason: 'no_active_credential' } — no active row
 *   { ok: false, reason: 'already_marked', credentialId, needsRefreshAt }
 *     — marker was already set; this call is idempotent and returns
 *     the existing timestamp so the caller can decide whether to
 *     re-emit the credential.rotated event or not.
 */
async function markForRotation(supabase, args) {
  if (!supabase || typeof supabase.from !== 'function') {
    throw new Error('markForRotation: supabase required');
  }
  if (!args || args.userId == null) {
    throw new Error('markForRotation: userId required');
  }
  const userId = Number(args.userId);
  const nowIso = new Date(Number.isFinite(args.nowMs) ? args.nowMs : Date.now()).toISOString();

  // Look up the active credential.
  const { data: active, error: lookupErr } = await supabase
    .from(TABLE)
    .select('id,needs_refresh_at')
    .eq('user_id', userId)
    .eq('status', 'active')
    .maybeSingle();

  if (lookupErr) {
    return { ok: false, reason: 'db_lookup_failed', dbError: lookupErr.message };
  }
  if (!active) {
    return { ok: false, reason: 'no_active_credential' };
  }

  if (active.needs_refresh_at) {
    // Idempotent: marker already set. Surface the existing value.
    return {
      ok:             false,
      reason:         'already_marked',
      credentialId:   active.id,
      needsRefreshAt: active.needs_refresh_at,
    };
  }

  const { error: updateErr } = await supabase
    .from(TABLE)
    .update({ needs_refresh_at: nowIso })
    .eq('id', active.id)
    .is('needs_refresh_at', null);

  if (updateErr) {
    return { ok: false, reason: 'db_update_failed', dbError: updateErr.message };
  }

  return {
    ok:             true,
    credentialId:   active.id,
    needsRefreshAt: nowIso,
  };
}

/**
 * Gated rotation — used by the LB-facing /credentials/refresh endpoint.
 *
 * Requires the bearer's credential to be (a) status='active' AND
 * (b) needs_refresh_at IS NOT NULL. Both conditions enforced atomically
 * via the UPDATE filter so two racing /refresh calls deterministically
 * select only ONE caller.
 *
 * Returns:
 *   { ok: true, token, newCredentialId, newTokenPrefix,
 *     previousCredentialId, previousGraceExpiresAt, expiresAt, issuedAt }
 *   { ok: false, reason }
 *     reason ∈ { 'no_pending_rotation', 'already_rotated_this_cycle',
 *                'credential_revoked', 'unknown_credential',
 *                'db_lookup_failed', 'db_update_failed',
 *                'mint_failed:<inner_reason>', 'invalid_arguments' }
 *
 * Distinction vs rotateCredential (force-rotate path):
 *   - rotateCredential: rotates any active credential unconditionally
 *     (used by admin force-rotate). Does NOT check needs_refresh_at.
 *   - performRefreshRotation: rotates ONLY if needs_refresh_at set on
 *     the presented bearer's credential. Used by LB /refresh.
 */
async function performRefreshRotation(supabase, args) {
  if (!supabase || typeof supabase.from !== 'function') {
    throw new Error('performRefreshRotation: supabase required');
  }
  if (!args || args.bearerCredentialId == null || args.userId == null) {
    return { ok: false, reason: 'invalid_arguments' };
  }
  const userId       = Number(args.userId);
  const bearerCredId = Number(args.bearerCredentialId);
  const reason       = args.reason || 'lb_refresh';
  const kid          = args.kid || getCurrentKid();
  const nowMs        = Number.isFinite(args.nowMs) ? args.nowMs : Date.now();
  const graceIso     = new Date(nowMs + GRACE_WINDOW_MS).toISOString();

  // Look up the bearer's credential row by id (already validated by
  // auth middleware, but we re-check status + marker here atomically).
  const { data: bearer, error: lookupErr } = await supabase
    .from(TABLE)
    .select('id,user_id,status,needs_refresh_at')
    .eq('id', bearerCredId)
    .maybeSingle();
  ...
  if (Number(bearer.user_id) !== userId) {
    // Defensive: should never happen if auth was correct.
    return { ok: false, reason: 'tenant_mismatch' };
  }
  if (bearer.status === 'revoked') {
    return { ok: false, reason: 'credential_revoked' };
  }
  if (bearer.status === 'rotating') {
    return { ok: false, reason: 'already_rotated_this_cycle' };
  }
  if (bearer.status !== 'active') {
    return { ok: false, reason: 'credential_revoked' };
  }
  if (!bearer.needs_refresh_at) {
    return { ok: false, reason: 'no_pending_rotation' };
  }

  // Atomically demote: active + marker set → rotating + clear marker.
  // Filtering by id + status='active' + needs_refresh_at IS NOT NULL
  // ensures only one racing caller wins.
  const { data: demoted, error: demoteErr } = await supabase
    .from(TABLE)
    .update({
      status:           'rotating',
      grace_expires_at: graceIso,
      needs_refresh_at: null,
    })
    .eq('id', bearer.id)
    .eq('status', 'active')
    .not('needs_refresh_at', 'is', null)
    .select('id')
    .maybeSingle();
  ...
  if (!demoted) {
    // Lost a race — another caller already won.
    return { ok: false, reason: 'already_rotated_this_cycle' };
  }

  // Mint the new active credential.
  const minted = await mintCredential(supabase, {
    userId,
    kid,
    createdBy: `refresh:${reason}`,
  });
  if (!minted.ok) {
    return { ok: false, reason: `mint_failed:${minted.reason}` };
  }

  // Best-effort: link new -> old via rotated_from_id.
  await supabase
    .from(TABLE)
    .update({ rotated_from_id: demoted.id })
    .eq('id', minted.credentialId)
