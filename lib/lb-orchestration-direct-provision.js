...
async function provisionWithLb(args) {
  ...
  // LB's /provision contract (verified end-to-end against staging):
  //   Request body: { link_token, provisioning: <oauth-payload-shape> }
  //     - `link_token` is top-level (single-use, returned by /verify-credentials)
  //     - `provisioning` wraps the same shape as the OAuth /oauth/exchange
  //       response (lib/lb-orchestration-provisioning-payload.js): version,
  //       tenant, credential, endpoints, signature_metadata, event_types.
  //     - No `webhook` block in the request — LB generates the webhook
  //       config and returns it in the response.
  //   Response: { ok, connection_id, sf_tenant_id, lb_user_id, webhook:{url,secret} }
  ...
  if (!res.ok) {
    ...
    return {
      ...
      status:           res.status,
      reason:           code,
      // LB uses `detail` for sub-error info; fall through to
      // error_description for forward-compat.
      errorDescription: data.detail || data.error_description || null,
    };
  }
  const d = res.data || {};
  const wh = (d.webhook && typeof d.webhook === 'object') ? d.webhook : null;
  if (!wh || !wh.url || !wh.secret || !d.lb_user_id) {
    return { ok: false, status: res.status, reason: 'lb_provision_malformed_response' };
  }
  return {
    ok:             true,
    webhookUrl:     String(wh.url),
    webhookSecret:  String(wh.secret),
    // LB returns connection_id; surface it as subscription_id since SF
    // stores it in lb_orchestration_subscription_id. LB has no separate
    // state_ref concept yet.
    subscriptionId: d.connection_id || null,
    stateRef:       d.state_ref     || null,
    lbAccountId:    String(d.lb_user_id),
    lbAccountName:  d.lb_user_email || d.lb_user_display_name || null,
  };
}
...
async function performDirectProvision(supabase, args) {
  ...
  if (settingRow.lb_orchestration_enabled_at) {
    // Surface the silent-skip in logs so future occurrences are visible
    // in Loki. The /connect-side stale-state clear added in 2026-06-03
    // should make this branch rare; if it fires for a tenant with
    // leadbridge_connected=true the caller already had a valid
    // provisioning, so the no-op is correct.
    try {
      logger.warn(
        `[orch-direct] preflight skip tenant=${tenantId} reason=already_provisioned ` +
        `lb_orchestration_enabled_at=${settingRow.lb_orchestration_enabled_at} ` +
