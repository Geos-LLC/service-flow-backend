...
async function logAmbiguity(supabase, input, source, candidates, reason, autoMerged = false) {
  ...
  // Opt-out for callers that want to write their own, source-specific
  // ambiguity row instead (e.g., zenbooker-sync.js writes
  // reason='zb_customer_import_ambiguous' with ZB-customer-shaped
  // source_payload). Without this opt-out the resolver's generic row
  // and the caller's specific row would both land — noisy queue.
  if (input.suppressAmbiguityLog === true) return;
  ...
  const attemptedExternalId = (input.externalId && String(input.externalId).trim()) || null;

  // Dedupe — if an ambiguity with the same (user, source, reason, status='open',
  // attempted_phone, attempted_external_id) is already queued, skip the insert.
  //
  // Including attempted_external_id in the dedupe key fixes a real bug: previously,
  // two distinct source records sharing a phone (e.g. ZB customers A and B both
  // with phone X that conflict with multiple SF customers) would collapse onto a
  // single ambiguity row — the second-arriving record would silently drop with no
  // queue entry, making it invisible to operators.
  if (status === 'open') {
    try {
      ...
      q = attemptedExternalId ? q.eq('attempted_external_id', attemptedExternalId) : q.is('attempted_external_id', null);
      const { count } = await q;
      if ((count || 0) > 0) return;
