    ...
    for (const c of custs) {
      ...
      try {
        ...
        if (apply && res.identity?.id && res.identity.sf_customer_id !== c.id) {
          ...
          // @transitional — backfill writes sf_customer_id directly. See doc above.
...
async function runIdentityBackfill(supabase, userId, { apply = false, progress = null, logger = null } = {}) {
  ...
  // identity rows, bypassing setIdentityCustomer/setIdentityLead. This is
  // intentional: backfill operates in strict mode on historic data, where
  // the projection cascade would be a no-op (no fresh source event). Emit
  // once per run so we can audit when/where backfill ran.
