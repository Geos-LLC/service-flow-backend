...
/**
 * Extract the snapshot from an existing row's metadata. Prefers the canonical
 * fields, falls back to legacy keys when the row predates §3.5. Returns null
 * when no rate signal can be recovered at all — in that case the caller MUST
 * fall back to historical pay-rate lookup before using current rates.
 *
