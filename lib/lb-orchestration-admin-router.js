...
const { performRotation, performMarkForRotation } = require('./lb-orchestration-handshake');
...
function makeAdminCredentialRouter(deps) {
  ...
  router.post('/credentials/rotate', ...gate, async (req, res) => {
    ...
    try {
      // performRotation wraps rotateCredential and additionally enqueues
      // the credential.rotated webhook event to lb_orchestration_outbox.
      // The drainer picks it up + delivers to the per-tenant webhook URL.
      const out = await performRotation(supabase, { userId: Number(userId), reason, logger });
      if (!out.ok) {
      ...
      try { logger.log(`[orch-admin] rotate ok user=${userId} old=${out.previousCredentialId} new=${out.newCredentialId} prefix=${out.newTokenPrefix} event=${out.event_id || 'none'}`); } catch (_) {}
      return res.status(200).json({
        token:                       out.token,
        ...
        event_enqueued:              out.event_enqueued,
      });
    } catch (err) {
  ...
  // ─────────────────────────────────────────────────────────────────
  // R1B — POST /credentials/mark_for_rotation
  // body: { user_id, reason? }
  //
  // Sets needs_refresh_at on the tenant's active credential AND
  // enqueues the credential.rotated webhook (refresh_required: true).
  //
  // Does NOT mint a new credential. The actual rotation happens when
  // LB calls /api/integrations/leadbridge/orchestration/credentials/refresh
  // using the current bearer.
  //
  // Distinguished from POST /credentials/rotate:
  //   - rotate (this file, above): immediate force-rotate. Returns
  //     plaintext to admin. Admin path for incident response.
  //   - mark_for_rotation (this endpoint): notify-only. No plaintext
  //     ever flows through this path. The pull-style /refresh flow on
  //     the LB side is the only place plaintext is delivered.
  //
  // 200: marker set + event enqueued
  // 404: no_active_credential (operator should re-handshake first)
  // 503: service_unavailable (transient DB error)
  // ─────────────────────────────────────────────────────────────────
  router.post('/credentials/mark_for_rotation', ...gate, async (req, res) => {
    const userId = req.body?.user_id;
    const reason = req.body?.reason || 'operator_request';
    ...
    try {
      const out = await performMarkForRotation(supabase, { userId: Number(userId), reason, logger });
      if (!out.ok) {
        if (out.reason === 'no_active_credential') {
          return res.status(404).json({ error: 'no_active_credential' });
        }
        try { logger.error(`[orch-admin] mark_for_rotation failed user=${userId} reason=${out.reason}`); } catch (_) {}
        return res.status(503).json({ error: 'service_unavailable', reason: out.reason });
      }

      try { logger.log(`[orch-admin] mark_for_rotation ok user=${userId} cred=${out.credentialId} event=${out.event_id || 'none'} already_marked=${!!out.alreadyMarked}`); } catch (_) {}
      return res.status(200).json({
        user_id:               Number(userId),
        active_credential_id:  out.credentialId,
        needs_refresh_at:      out.needsRefreshAt,
        already_marked:        !!out.alreadyMarked,
        event_id:              out.event_id,
        event_enqueued:        !!out.event_enqueued,
        reason,
      });
    } catch (err) {
      try { logger.error(`[orch-admin] mark_for_rotation threw: ${err && err.message}`); } catch (_) {}
      return res.status(500).json({ error: 'internal_error' });
    }
  });
